...
# set command to display and set shell variable
# -e return value not equal zero, then exit from shell.
set -e
# define variable "= ", and use by $
...
# tee  tee命令作用可以用字母T来形象地表示。它把输出的一个副本输送到标准输出，另一个副本拷贝到相应的文件中。如果希望在看到输出的同时，也将其存入一个文件，那么这个命令再合适不过了。
#      它的一般形式为：tee -a files 其中，- a表示追加到文件末尾。
tee <$npipe $logfile &
# bash内置命令 0表示标准输入,1是标准输出,2是错误输出,3-9是空白描述符
exec 1>&-
exec 1>$npipe 2>&1
# trap是一个可以设置接收信号类型，然后执行你给出的命令的命令
