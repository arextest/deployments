...
if ! [ -x "$(command -v docker)" ]; then
  echo "Error: Docker is not installed. install Docker services please." >&2
  exit 1
fi

# Check if Docker Compose is installed
if ! [ -x "$(command -v docker compose)" ]; then
  echo "Error: Docker Compose is not installed. install Docker compose services please." >&2
  exit 1
fi
